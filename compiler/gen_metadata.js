...
exports.toSimpleType = void 0;
const ts = require("typescript");
const fs = require("fs");
const commandLineArgs = require("command-line-args");
const AllTypes = require("./types/AllTypes");
const AllMetadata = require("./metadata/AllMetadata");
function toSimpleType(typeNode) {
    switch (typeNode.kind) {
        ...
        case ts.SyntaxKind.TypeReference:
            ...
            let args = typeNode.typeArguments
                ? typeNode.typeArguments.map(t => toSimpleType(typeNode.typeArguments[0]))
                : [];
            return new AllTypes.TypeReferenceType(typeNode.typeName.escapedText, args);
        case ts.SyntaxKind.FunctionType:
            // generics = (typeNode.typeParameters || []).map(p => new AllTypes.SimpleType(p))
            return new AllTypes.FunctionType(typeNode.parameters.map(p => toSimpleType(p)), toSimpleType(typeNode.type));
        ...
        case ts.SyntaxKind.TupleType:
            return new AllTypes.TupleType(typeNode.elementTypes.map(t => toSimpleType(t)));
        case ts.SyntaxKind.UnionType:
            return new AllTypes.UnionType(typeNode.types.map(t => toSimpleType(t)));
            break;
...
function isNodeHidden(sourceFile, node) {
    return (sourceFile &&
        node.jsDoc &&
        !!node.jsDoc.find(doc => sourceFile.text.slice(doc.pos, doc.end).search("@hidden") != -1));
}
function visit(checker, sourceFile, file, node) {
        ...
        switch (node.kind) {
            case ts.SyntaxKind.FunctionDeclaration:
                ...
                let doc = ts.displayPartsToString(symbol.getDocumentationComment()) ||
                    "";
                // Grab the type
                let ttype = checker.getTypeOfSymbolAtLocation(symbol, symbol.valueDeclaration);
                ...
                file.setFunction(nodeName, new AllMetadata.SymbolMetadata(doc, t, isNodeHidden(sourceFile, node)));
                return;
            case ts.SyntaxKind.ClassDeclaration:
                ...
                symbol.members.forEach((sym, name, map) => {
                    ...
                    if (["__constructor", "get", "set"].includes(name))
                        return;
                    // Grab the doc, default to empty string
                    let doc = ts.displayPartsToString(sym.getDocumentationComment()) || "";
                    // Grab the type
                    let ttype = checker.getTypeOfSymbolAtLocation(sym, sym.valueDeclaration);
                    // If the function has a type, try to convert it, if it doesn't, default to any
                    let t = ttype
                        ? toSimpleType(checker.typeToTypeNode(ttype))
                        : new AllTypes.AnyType();
                    clazz.setMember(name, new AllMetadata.SymbolMetadata(doc, t, isNodeHidden(sourceFile, node)));
                });
                return;
...
function generateMetadata(out, themedir, fileNames) {
    /* Parse Tridactyl */
    ...
    for (const sourceFile of program.getSourceFiles()) {
        let name = fileNames.find(name => sourceFile.fileName.match(name));
        if (name) {
...
if (!opts.out || opts.src.length < 1)
    throw new Error("Argument syntax: --out outfile [--src] file1.ts [file2.ts ...]");
